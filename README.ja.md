...
システムはすべてを `machine` 検証に向けて推進します。マルチモデルコードレビュー（Codex + Gemini + Claude）が独立して実行され、合意された判定を合成します。独立した検証者が隔離されたコンテキストで Definition of Done をチェックし、自己検証バイアスを排除します。
...
- **`spec search`** — 全スペックを横断する BM25 検索：要件、シナリオ、制約、学習。プロジェクト A で学んだことがプロジェクト B での質問に反映される

...
시스템은 모든 것을 `machine` 검증 쪽으로 밀어붙인다. 다중 모델 코드 리뷰(Codex + Gemini + Claude)가 독립적으로 실행되어 합의된 판정을 내린다. 독립 검증자가 격리된 컨텍스트에서 완료 기준을 점검하여 자기 검증 편향을 제거한다.
...
- **`spec search`** — 모든 스펙을 BM25 검색: 요구사항, 시나리오, 제약조건, 학습. 프로젝트 A에서 배운 것이 프로젝트 B의 질문에 반영

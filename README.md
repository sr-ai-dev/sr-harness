...
`sr-harness-cli` manages plan.json validation and task state:

```bash
sr-harness-cli plan get <task-id> <plan-path>                    # Get task details
sr-harness-cli plan task <plan-path> --status <task-id>=done   # Update task state
```

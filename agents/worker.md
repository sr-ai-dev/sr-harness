...
- You may call `sr-harness-cli plan get <plan-path> --path tasks` to re-fetch the task list if needed. Do **not** read `plan.json` directly with Read/Edit.
- You must **never** write to `plan.json`. Status updates go through `sr-harness-cli plan task <plan-path> --status <task-id>=<pending|running|done|failed|blocked> --summary "<msg>"`.
...
2. Run:
   ```bash
   sr-harness-cli plan task <plan-path> \
...
**Completion condition**: All sub-requirement GWT scenarios satisfied AND build/lint passes (AND tests pass in TDD mode). On success, mark the task done via `sr-harness-cli plan task <plan-path> --status <task-id>=done --summary "<one-line summary>"`.
...
5. **plan.json is read-only for you**: Use `sr-harness-cli plan get` to read and `sr-harness-cli plan task --status <id>=<state>` to update task state. Never open `plan.json` with Read/Edit/Write. Do not use the legacy `sr-harness-cli spec task` / `spec derive-tasks` commands — those are v1 and no longer apply.

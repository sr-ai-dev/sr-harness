...
Claude Code의 CLAUDE.md, Rules, Skills, Agents는 모두 LLM에게 주입되는 지시라는 점에서는 프롬프트다. 하지만 실제 제어력은 프롬프트 자체가 아니라, **언제 로드하고, 어떤 도구를 허용하며, 어떤 결과를 거부하는지 결정하는 런타임 하네스**에서 나온다.

이 문서의 핵심 질문은 세 가지다:

1. 어떤 요소가 단순 프롬프트인가?
2. 어떤 요소가 런타임에서 강제되는가?
3. 이 구조를 Claude Code 밖으로 옮길 때 무엇이 남고 무엇이 사라지는가?

결론적으로, 좋은 하네스는 LLM에게 더 긴 지시를 주는 시스템이 아니다. **지식은 프롬프트로 전달하고, 상태는 파일에 남기며, 강제는 CLI/Hook/도구 제한으로 처리하는 구조**가 좋은 하네스다.
...
Claude Code의 구성 요소는 모두 같은 강도로 LLM을 제어하지 않는다. 프롬프트에 가까울수록 LLM의 준수도에 의존하고, 런타임에 가까울수록 물리적 제약과 결정론적 검증에 가까워진다.

| 단계 | 방식 | 예시 | 제어력 |
|------|------|------|:------:|
| 1 | 일반 프롬프트 | "코드를 수정하지 마라" | 낮음 |
| 2 | 항상 로드되는 규칙 | CLAUDE.md, `paths` 없는 Rule | 낮음~중간 |
| 3 | 조건부 로딩 | `paths` 있는 Rule, Skill lazy load | 중간 |
| 4 | 도구 제한 | `allowed-tools`, `disallowed-tools` | 높음 |
| 5 | 격리 실행 | Agent 별도 컨텍스트 | 높음 |
| 6 | 결정론적 검증 | CLI validate, Hook, schema check | 매우 높음 |

이 사다리를 기준으로 보면, 문서 전체의 흐름은 단순하다. **CLAUDE.md와 Rules는 지시를 주고, Skill은 절차를 주며, Agent는 역할과 컨텍스트를 분리하고, Hook/CLI는 결과를 검증한다.**
...
### 2.3 관통 예시: `/specify`에서 `design.md`까지

이 문서의 각 장은 하나의 흐름으로 묶어 이해할 수 있다. 예를 들어 `/specify`로 요구사항을 도출하고 `design.md`를 생성하는 작업은 다음처럼 진행된다:

```
CLAUDE.md
  → 세션 전체의 말투, 작업 원칙, 프로젝트 기본 규칙 제공

Rules
  → 특정 문서 파일 접근 시 동기화/검토 규칙 주입

Skill (/specify)
  → L0 목표 → L1 컨텍스트 → L2 결정 → L3 요구사항 → L4 태스크 절차 제공

Agent
  → reviewer/debugger/worker가 별도 컨텍스트에서 분석 또는 검토

CLI / Hook
  → spec.json 스키마 검증, 단계 전환, 산출물 생성 확인

design.md
  → 사람이 읽고 리뷰할 수 있는 협업용 문서로 렌더링
```

즉, 하나의 기능 개발 흐름 안에서도 각 장치는 역할이 다르다. **프롬프트는 판단 기준을 전달하고, 파일은 상태를 보존하며, 런타임은 제약과 검증을 담당한다.**
...
| 오해 | 실제 |
|------|------|
| Skill은 프로그램이다 | 아니다. LLM이 읽고 따르는 절차 프롬프트다 |
| Agent는 단순 역할극이다 | 아니다. 별도 컨텍스트와 도구 제한을 가진 실행 단위다 |
| Rules는 항상 시스템이 강제한다 | 아니다. `paths` 없는 Rule은 LLM 판단에 의존한다 |
| Lazy loading은 컨텍스트를 제거한다 | 아니다. 늦게 넣을 뿐, 한번 들어오면 누적된다 |
| `allowed-tools`는 권고다 | 아니다. 런타임이 도구 목록을 제한하므로 물리적 제약에 가깝다 |
| 범용 LLM에는 이식이 불가능하다 | 직접 이식은 불가능하지만 지식과 구조는 이식 가능하다 |

이 구분이 중요한 이유는 설계 책임을 잘못 배치하지 않기 위해서다. LLM이 "잘 지켜주길 바라는" 규칙은 프롬프트에 둘 수 있지만, 반드시 보장해야 하는 조건은 CLI, Hook, schema, 도구 제한 같은 런타임 장치로 내려야 한다.
...
## 7. 프레임워크 3자 비교: superpowers vs GSD vs hoyeon
...
> 외부 프로젝트의 스타 수, 지원 런타임, 기능 범위는 문서 작성 시점 기준이다. 배포나 도입 의사결정에 사용하기 전에는 최신 README, 릴리즈 노트, 설치 문서를 다시 확인해야 한다.
...
GSD v2는 독립 CLI(Pi SDK 기반)이므로 Claude Code에 종속되지 않는다. §6의 3-Layer 모델로 보면, GSD v2는 **Layer 3(인프라)를 자체 CLI로 해결**하여 플랫폼 독립성을 확보했다.
...
## 8. hoyeon의 강점과 약점 — 하네스 엔지니어링 관점

### 8.1 2026년 하네스 엔지니어링의 핵심 원칙
...
### 9.3 이 프로젝트에서 무엇을 어디에 둘 것인가

하네스 설계에서 중요한 것은 모든 지시를 CLAUDE.md나 Skill에 몰아넣지 않는 것이다. 규칙의 성격에 따라 배치 위치를 달리해야 한다.

| 필요 | 적합한 장치 | 이유 |
|------|-------------|------|
| 항상 지켜야 하는 말투/작업 원칙 | CLAUDE.md | 세션 전체에 적용되는 기본 정책 |
| 특정 파일군 작업 시만 필요한 규칙 | `paths` 있는 Rule | 토큰 절약 + 조건부 적용 |
| 반복되는 작업 절차 | Skill | 절차를 필요 시 로드 |
| 독립 검토/분석 | Agent | 컨텍스트 격리 + 역할 제한 |
| 결과 형식 강제 | CLI validate / Hook | LLM 판단이 아닌 결정론적 검증 |
| 사람 읽기용 산출물 | `design.md` 렌더링 | `spec.json`의 협업 가독성 보완 |

### 9.4 실행 가능한 보완 액션

이 문서를 기준으로 이 프로젝트의 하네스를 개선한다면 우선순위는 다음과 같다:

1. `.claude/rules/doc-sync-propagation.md`에 `paths` frontmatter를 추가하여 특정 문서 접근 시에만 동기화 규칙이 로드되도록 한다.
2. `spec.json → design.md` 렌더링 규칙을 Layer 1(문서 작성 기준), Layer 2(생성/리뷰 흐름), Layer 3(CLI/Hook 구현)로 분리해 문서화한다.
3. 장기 작업에서 context rot를 줄이기 위해 태스크 단위 fresh context 실행 또는 worker agent 분리를 검토한다.
4. 외부 프레임워크 비교표는 정기적으로 최신 README 기준으로 갱신한다.
5. 사람이 읽는 문서와 기계가 검증하는 JSON의 역할을 분리하되, 두 산출물이 같은 ID 체계를 공유하도록 유지한다.

---

## 10. 결론: 하네스 설계 원칙

좋은 AI 개발 하네스는 LLM에게 더 긴 지시를 주는 시스템이 아니다. 좋은 하네스는 **지식, 상태, 강제**를 분리한다.

| 구분 | 저장 위치 | 이유 |
|------|-----------|------|
| 지식 | CLAUDE.md, Rules, Skill, Agent prompt | LLM이 판단할 기준을 제공 |
| 상태 | `spec.json`, `design.md`, repo files | 세션이 바뀌어도 보존되는 진실의 원천 |
| 강제 | CLI, Hook, schema, tool gate | LLM 준수도에 의존하지 않는 검증과 제약 |

프롬프트는 지식을 전달하는 데 강하고, 파일은 상태를 보존하는 데 강하며, CLI/Hook/도구 제한은 강제에 적합하다. 이 세 가지를 섞지 않는 것이 이 프로젝트의 하네스 설계 원칙이다.
...
| **00_skill-architecture-understanding.md** | **이 문서 (LLM 제어 방식 + 이식성 + 프레임워크 비교 + 업계 분석)** |

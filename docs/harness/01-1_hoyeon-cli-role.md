# hoyeon-cli의 역할과 스킬 의존성

> 작성일: 2026-04-12
> 목적: hoyeon 생태계에서 hoyeon-cli의 정확한 역할과 스킬과의 의존 관계 정리

---

## 1. hoyeon-cli란

`hoyeon-cli`는 **npm으로 전역 설치하는 Node.js CLI 도구**이다. Claude Code 밖에서 독립 실행되는 프로세스로, spec.json 파일을 안전하게 조작하는 전용 도구이다.

```bash
npm install -g @team-attention/hoyeon-cli
```

- **패키지명**: `@team-attention/hoyeon-cli`
...
│    │                                                │
│    └─ Bash("hoyeon-cli spec merge ... --stdin << EOF │
│         {\"context\": {\"decisions\": [...]}}         │
│         EOF")                                       │
│         ↓                                           │
│    ┌────────────────────────┐                       │
...
│    │                                                │
│    ├─ "L3 요구사항을 도출하자"                         │
│    └─ Bash("hoyeon-cli spec derive-requirements ...") │
│         ↓                                           │
│    └─ Bash("hoyeon-cli spec derive-tasks ...")       │
│                                                     │
│  /execute (SKILL.md)                                │
│    │                                                │
│    ├─ Bash("hoyeon-cli spec plan ...")  → 실행 계획   │
│    ├─ Bash("hoyeon-cli spec task T1 --status done")  │
│    └─ Bash("hoyeon-cli spec check ...")  → 일관성 검증│
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
- **hoyeon-cli** = "spec.json을 안전하게 조작하는 도구" (JavaScript)
- 스킬이 **Bash 도구를 통해** hoyeon-cli를 호출한다
- **hoyeon-cli 없이 스킬이 동작할 수 없다** — 필수 의존성
...
| hoyeon-cli (npm 패키지) | `npm install -g @team-attention/hoyeon-cli` | **필수** — 없으면 스킬 동작 불가 |
| Node.js | 시스템 설치 | 필수 (hoyeon-cli 실행 환경) |
...
/specify 스킬 (Claude Code 내)
    ↓ Bash("hoyeon-cli spec init ...")
.hoyeon/specs/<name>/spec.json  ← 생성
    ↓ Bash("hoyeon-cli spec merge ... --stdin")
spec.json  ← decisions, requirements, tasks 병합
    ↓ Bash("hoyeon-cli spec validate ...")
스키마 검증 + 커버리지 체크
    ↓ Bash("hoyeon-cli spec plan ...")
...
spec.json  ← 태스크 상태 업데이트
    ↓ Bash("hoyeon-cli spec learning --task T1 ...")
context/learnings.json  ← 교훈 기록
    ↓ Bash("hoyeon-cli spec check ...")
최종 일관성 검증
```
...
| 관점 | 현재 (글로벌 스킬) | hoyeon |
|------|-------------------|--------|
| **산출물 작성** | LLM이 직접 마크다운 작성 | LLM이 hoyeon-cli를 통해 JSON 작성 |
| **검증** | 사람이 읽고 판단 | AJV 스키마 자동 검증 |
| **외부 도구 의존** | 없음 | hoyeon-cli (npm) 필수 |

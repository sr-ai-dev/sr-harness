...
| 에이전트 | 23개 |
| Hook | 13개 |
| CLI | @syscon-robotics/sr-harness-cli (npm) |
...
```
~/.claude/
├── settings.json                              ← enabledPlugins: hoyeon@syscon-robotics-dev
├── plugins/
...
|------|---------------|-----------------|
| 내부 디렉토리 | `.sr-harness/` | `.sr-harness/` |
| CLI 바이너리 | `sr-harness-cli` | `sr-harness-cli` |
...
# 2. 디렉토리명 변환: .sr-harness → .sr-harness
find . -type d -name ".sr-harness" | while read dir; do
  mv "$dir" "$(dirname "$dir")/.sr-harness"
done
find . -type f \( -name "*.md" -o -name "*.sh" -o -name "*.json" -o -name "*.js" -o -name "*.ts" \) \
  -exec sed -i '' 's/\.sr-harness/\.sr-harness/g' {} +

# 3. CLI 바이너리명 변환: sr-harness-cli → sr-harness-cli
find . -type f \( -name "*.md" -o -name "*.sh" -o -name "*.json" -o -name "*.js" -o -name "*.ts" \) \
  -exec sed -i '' 's/sr-harness-cli/sr-harness-cli/g' {} +

# 4. npm 패키지 scope 변환
find . -type f -name "*.json" \
  -exec sed -i '' 's/@syscon-robotics\/sr-harness-cli/@syscon-robotics\/sr-harness-cli/g' {} +
...
- [x] release 브랜치 네이밍 검증 (sr-harness-cli→sr-harness-cli, .sr-harness→.sr-harness, plugin name→sr-harness)
...
| 작업 디렉토리 | `.sr-harness/` | `.sr-harness/` |
| CLI | `sr-harness-cli` | `sr-harness-cli` |
| npm scope | `@syscon-robotics` | `@syscon-robotics` |
...
```
1. release 브랜치 생성 + 네이밍 변환 (.sr-harness→.sr-harness 등)
2. 잔여 참조 검사
3. 커밋
...
| develop | `.sr-harness`, `sr-harness-cli` | ✅ | 개발자 내부 확인용 |
...
|----------|-----------------|-----------------|
| 디렉토리/참조 | `.sr-harness` | `.sr-harness` |
| CLI 바이너리 | `sr-harness-cli` | `sr-harness-cli` |
| npm scope | `@syscon-robotics` | `@syscon-robotics` |
| plugin name | `hoyeon` | `sr-harness` |

예: `/execute` 스킬에 `sr-harness-cli spec validate .sr-harness/specs/...`라고 적혀 있으면, release에서는 자동으로 `sr-harness-cli spec validate .sr-harness/specs/...`로 변환된다.
...
> `.sr-harness` 네이밍이 보이지만 실행에는 문제 없다.

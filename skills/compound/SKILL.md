...
2. **Extract Plan path**
   - Find Plan path pattern in PR body: `.sr-harness/specs/{name}/PLAN.md`
   - Regex: `\.sr-harness/specs/[^/]+/PLAN\.md`
   - **If no Plan path found**: Prompt user to enter spec name directly or select from `.sr-harness/specs/` directory listing

3. **Derive Context path**
   - Extract spec name from Plan path
   - Context directory: `.sr-harness/specs/{name}/context/`
...
4. **Parallel collection** (run following commands simultaneously, skip if files don't exist)
   ...
   cat .sr-harness/specs/{name}/context/learnings.json 2>/dev/null || echo ""
   cat .sr-harness/specs/{name}/context/decisions.md 2>/dev/null || echo ""
   cat .sr-harness/specs/{name}/context/issues.json 2>/dev/null || echo ""

...
sr-harness-cli session set --sid $SESSION_ID --json '{"council": {"phase": 1, "mode": "[selected]", "topic": "[topic summary]", "status": "active"}}'
```
...
```bash
sr-harness-cli session set --sid $SESSION_ID --json '{"council": {"phase": 2, "status": "active", "cycle": [N]}}'
```
...
```bash
sr-harness-cli session set --sid $SESSION_ID --json '{"council": {"phase": 3, "status": "completed"}}'
```

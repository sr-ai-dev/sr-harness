...
AskUserQuestion(
  ...
  header: "Next step",
  options: [
    { label: "Save insights", description: "Save to .sr-harness/deep-interview/[topic]/insights.md" },
...
```
Write(".sr-harness/deep-interview/[topic-slug]/insights.md", insights_content)
```

Use the **insights.md template** (see below). After saving, re-present the Next Steps question (without "Save insights").

#### Hand off to /specify
1. Save insights to `.sr-harness/deep-interview/[topic-slug]/insights.md` (if not already saved)
2. Generate the handoff command:
```
"Ready to plan. Run:
/specify --context .sr-harness/deep-interview/[topic-slug]/insights.md \"[1-line topic summary]\""
```
3. Stop
...
18. Save insights + generate:
    /specify --context .sr-harness/deep-interview/todo-cli/insights.md "Todo management CLI — local JSON-based CRUD + priorities/due dates/tags"
```

...
AskUserQuestion(
  ...
  header: "Next step",
  options: [
    { label: "Save insights", description: "Save to .sr-harness/discuss/[topic]/insights.md for future reference" },
...
```
Write(".sr-harness/discuss/[topic-slug]/insights.md", insights_content)
```

Use the **insights.md template** (see below). After saving, re-present the Next Steps question (without "Save insights").

#### Hand off to /specify
1. Save insights to `.sr-harness/discuss/[topic-slug]/insights.md` (if not already saved)
2. Generate the handoff command:
```
"Ready to plan. Run:
/specify --context .sr-harness/discuss/[topic-slug]/insights.md \"[1-line topic summary]\""
```
3. Stop
...
14. Save insights + generate: /specify --context .sr-harness/discuss/payment-rewrite/insights.md "Improve payment module reliability"
```

...
3. **plan.json is the ledger** — Task state via `sr-harness-cli plan` commands. Never direct file writes.
...
IF input_mode == "plan":                             # R-F1.1
  ...
  Bash("sr-harness-cli plan validate {spec_dir}")
...
ELIF input_mode == "virtual":                        # R-F1.3
  ...
  Bash("sr-harness-cli plan init {spec_dir} --type {draft_plan.meta.type}")
  Bash("sr-harness-cli plan merge {spec_dir} --json \"$(cat /tmp/plan-virtual.json)\"")
  Bash("sr-harness-cli plan validate {spec_dir}")
```
...
**MAY read** (structural, via `sr-harness-cli plan get` or `Read` on plan.json only):
- `plan.json` fields: `tasks`, `journeys`, `verify_plan`, `meta`, `contracts.artifact`, `context`
- Session state in `$HOME/.sr-harness/$CLAUDE_SESSION_ID/state.json`
...
- `sr-harness-cli plan task --status X=done` is idempotent (INV-5, INV-9); re-issuing
...
- No `sr-harness-cli plan get` called in a loop to poll for `status == "done"`.
...
— never through `sr-harness-cli` (INV-5). See `references/contracts-patch.md` for
...
|-------------------|-------------------|------------------|--------------------------------------------------------------------------|
| `plan.json`       | orchestrator      | `sr-harness-cli`    | Status mutated via `plan task --status` (INV-5, INV-9; never direct edit) |
...
`sr-harness-cli` (INV-5 / R-N17.1). The contracts-patch recipe writes its own
...
Workers MUST NOT use `sr-harness-cli` to mutate either file (INV-5).
...
| Summary             | `sr-harness-cli plan list` counts + verify result tallies                 |
| Post-Work           | Computed from MANUAL gate=4 entries, PERSISTENT_GAP entries, BLOCKED tasks |
| Tasks               | `sr-harness-cli plan get <plan> --path tasks` + git log per round         |
...
1. **plan.json is the ledger** — all task CRUD via `sr-harness-cli plan` (status/list/get/merge). Never direct file writes (INV-5).
...
- [ ] Plan generated or validated via sr-harness-cli

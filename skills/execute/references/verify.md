...
- **INV-5**: `plan.json` is mutated only via `sr-harness-cli plan task --status`. Verify
...
done_tasks = Bash("sr-harness-cli plan list {spec_dir} --status done --json").tasks
...
`sr-harness-cli plan get`'s attached summaries.
...
IF gaps AND gap_retry_rounds < 1:
  ...
  FOR tid, sids in by_task.items():
    ...
    Bash("sr-harness-cli plan task --status {tid}=running")   # reset to running
...
IF failures AND fix_loop_rounds < 1:
  ...
  FOR tid, fails in by_task.items():
    ...
    Bash("sr-harness-cli plan task --status {tid}=running")

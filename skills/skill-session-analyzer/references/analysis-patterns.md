...
grep "\.sr-harness/drafts/" debug.txt | grep "written atomically"

# Pattern - draft deletion (look for rm command)
grep "rm.*\.sr-harness/drafts/" debug.txt
```

### Plan File Operations
```bash
# Pattern - plan creation
grep "\.sr-harness/specs/" debug.txt | grep "written atomically"
```
...
# 5. Check artifacts
grep -E "(\.sr-harness/drafts/|\.sr-harness/specs/).*written atomically" debug.txt

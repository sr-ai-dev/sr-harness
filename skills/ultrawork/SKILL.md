...
6. Write approved `spec.json` at `.sr-harness/specs/{name}/spec.json`
...
- Hook automatically injects `/execute .sr-harness/specs/{name}/spec.json`
...
The hook tracks progress in `.sr-harness/state.local.json`:
```json
{
  "session-id": {
    "ultrawork": {
...
[spec.json written and approved at .sr-harness/specs/dark-mode/spec.json]
[Hook detects approved spec → triggers "/execute .sr-harness/specs/dark-mode/spec.json"]
